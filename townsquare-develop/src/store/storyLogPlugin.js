...
const getPhaseEntry = (state, entry) => {
  const game = state.storyLog && state.storyLog.games[state.storyLog.currentGameId];
...
const remindersText = player =>
  (player && player.reminders ? player.reminders : []).map(describeReminder).join("|");
...
function handlePlayersSet(store, beforePlayers, afterPlayers) {
  ...
  const remindersChanged = beforePlayers.some(
    (player, index) => remindersText(player) !== remindersText(afterPlayers[index])
  );
...
function handlePlayerUpdate(store, beforePlayers, payload) {
  ...
  const before = beforePlayers.find(player => player.id === payload.player.id);
  ...
  if (payload.property === "isVoteless" && before.isVoteless !== payload.value) {
    addAutoEntry(store, {
      type: "status",
      title: `${playerName(after)} ${payload.value ? "失去投票权" : "恢复投票权"}`,
      playerIds: [after.id]
    });
  }
...
function handleVoteHistory(store, beforeHistory) {
  ...
  addAutoEntry(store, {
    ...
    content: latest.votes.length ? `投票玩家：${latest.votes.join("、")}` : "无人投票"
  });
}
  ...
  store.subscribe((mutation, state) => {
    ...
    switch (type) {
      ...
      case "setEdition":
        addAutoEntry(store, {
          type: "script",
          title: `选择剧本：${state.edition.name || payload.name || "自定义剧本"}`
        });
        break;
      case "players/add":
        addAutoEntry(store, {
      ...
      case "players/swap":
        addAutoEntry(store, {
          type: "player",
          title: `交换座位：${playerName(beforePlayers[payload[0]])} / ${playerName(
            beforePlayers[payload[1]]
          )}`
        });
        break;
      case "players/move":
