...
- **必须通过项（在 Phase 0 研究前）**：
  ...
  - **UI 设计规范**：若功能包含用户界面，计划 MUST 说明 UI 设计遵循的设计系统（本项目宪章要求严格参考 Ant Design），并列出需要设计交付物（组件映射、断点、交互状态）以及 UI 审核规则。
  - **管理端平台**：若计划实现管理端功能，Target Platform MUST 指定为浏览器（H5）并且实现/测试仅需覆盖 H5 环境；不要求兼容原生 App、小程序或桌面客户端，除非在计划中明确提出并获得宪章层面批准。
