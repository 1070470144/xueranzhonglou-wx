    ...
    <view class="uni-container">
      ...
      <view class="table-card">
        <scroll-view scroll-x class="table-scroll">
          <uni-table class="table" border stripe :loading="loading" emptyText="暂无网页公告">
            <uni-tr>
    ...
    <view v-if="editorVisible" class="editor-mask" @click="closeEditor">
      <view class="editor" @click.stop>
        <view class="editor-head">
          <text class="editor-title">{{ form._id ? '编辑网页公告' : '新增网页公告' }}</text>
          <button class="uni-button" size="mini" @click="closeEditor">关闭</button>
        </view>
        <scroll-view scroll-y class="editor-body">
          <view class="form-row">
            <text class="label">标题</text>
            <input class="input" v-model="form.title" placeholder="网页公告标题" />
          </view>
          <view class="form-row">
            <text class="label">摘要</text>
            <input class="input" v-model="form.summary" placeholder="列表和弹窗里展示的简短摘要" />
          </view>
          <view class="form-row">
            <text class="label">适用版本</text>
            <input class="input" v-model="form.version" placeholder="可空，例如 1.4.0" />
          </view>
          <view class="form-row">
          ...
          <view class="form-row form-row-top">
            ...
            <textarea class="textarea" v-model="form.content" placeholder="网页公告正文"></textarea>
          </view>
        </scroll-view>
...
export default {
  ...
  methods: {
    async load() {
      this.loading = true;
      try {
        const res = await listWebAnnouncements({ page: this.page, pageSize: this.pageSize, keyword: this.keyword, status: this.status, type: this.type });
    ...
    async save() {
      ...
      this.saving = true;
      try {
        const res = await saveWebAnnouncement(payload);
    ...
    changeStatus(item, status) {
      uni.showModal({
        title: '确认操作',
        content: `确定${status === 'published' ? '发布' : '下架'}该网页公告吗？`,
        success: async (res) => {
          if (!res.confirm) return;
          const result = await updateWebAnnouncementStatus({ id: item._id, status });
    ...
    remove(item) {
      uni.showModal({
        ...
        success: async (res) => {
          if (!res.confirm) return;
          const result = await deleteWebAnnouncement(item._id);
