...
### User Story 5 - JSON复制使用统计 (Priority: P2)

每次用户复制剧本JSON数据时，剧本的使用次数自动增加一次，并同步更新后台数据库。

**Why this priority**: 为剧本作者和管理员提供数据使用统计，便于了解剧本的受欢迎程度和使用情况。

**Independent Test**: 可以通过复制JSON后，验证使用次数是否正确增加并同步到数据库。

**Acceptance Scenarios**:

1. **Given** 用户在剧本详情页面，**When** 点击"复制JSON"按钮成功，**Then** 剧本使用次数自动增加1
2. **Given** 使用次数更新操作，**When** 执行成功，**Then** 数据库中的使用次数字段正确更新
3. **Given** 使用次数统计，**When** 多个用户复制同一剧本，**Then** 每次复制都正确累加使用次数
4. **Given** 使用次数更新失败，**When** 网络异常或数据库错误，**Then** 提供适当的用户反馈但不影响JSON复制功能
...
- **FR-009**: 小程序端MUST在每次成功复制JSON时自动增加剧本使用次数
- **FR-010**: 小程序端MUST同步更新后台数据库中的使用次数统计
...
- **SC-007**: 每次JSON复制成功后，使用次数准确增加并同步到后台数据库
- **SC-008**: 使用次数统计更新响应时间不超过1秒，不影响JSON复制主流程