...
### User Story 4 - 剧本详情复制JSON功能 (Priority: P2)

在剧本详情页面提供复制JSON数据的功能，点击后生成一个.json后缀的链接，可以在浏览器中直接打开查看完整的JSON内容。

**Why this priority**: 为开发者和高级用户提供便捷的数据导出和查看方式，便于调试和数据分析。

**Independent Test**: 可以通过点击复制JSON按钮，验证生成的链接能正确打开并显示剧本的完整JSON数据。

**Acceptance Scenarios**:

1. **Given** 用户在剧本详情页面，**When** 点击"复制JSON"按钮，**Then** 生成一个.json后缀的链接并自动复制到剪贴板
2. **Given** 生成的JSON链接，**When** 在浏览器中打开，**Then** 显示剧本的完整JSON数据结构
3. **Given** JSON数据包含所有剧本字段，**When** 查看JSON内容，**Then** 与数据库中的原始数据完全一致
...
- **FR-007**: 小程序端MUST在剧本详情页面提供复制JSON数据的功能
- **FR-008**: 小程序端MUST生成可直接在浏览器打开的.json链接
...
- **SC-005**: 复制JSON功能生成的链接在浏览器中能正确显示完整的剧本数据
- **SC-006**: JSON链接生成和复制操作响应时间不超过2秒