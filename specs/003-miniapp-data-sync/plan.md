...
适配现有小程序界面和数据结构，使其与管理端保持一致的数据字段定义，确保小程序能正常刷新和管理端数据库的数据。同时完善现有的剧本详情页面复制JSON功能，让其生成可直接在浏览器打开的.json链接，并实现每次复制JSON时自动增加使用次数的统计功能。重点是数据结构统一、同步机制、数据导出和使用统计功能的完善，不涉及界面重新设计。
...
└── uniCloud-aliyun/cloudfunctions/
    ...
    │   └── index.js
    └── getScriptJson/           # 完善现有云函数支持浏览器.json链接
        └── index.js
```
...
- getScriptJson API扩展：完善现有获取剧本JSON数据的接口，支持生成浏览器可直接访问的.json链接和使用次数统计
- incrementUsage API：新增剧本使用次数递增接口
- 响应格式标准化
...
- script-detail.vue：支持完整数据字段展示和JSON复制功能
- 数据映射和格式化处理

### JSON复制功能完善
- 修改getScriptJson云函数：完善现有云函数，支持生成浏览器可直接访问的.json链接和使用次数统计
- 新增incrementUsage云函数：实现剧本使用次数递增功能
- script-detail.vue：优化复制JSON按钮的链接生成逻辑，集成使用次数统计（按钮UI已存在）
- 完善剪贴板复制功能和浏览器链接生成
