...
适配现有小程序界面和数据结构，使其与管理端保持一致的数据字段定义，确保小程序能正常刷新和管理端数据库的数据。同时完善现有的剧本详情页面复制JSON功能，让其生成可直接在浏览器打开的.json链接。重点是数据结构统一、同步机制和数据导出功能的完善，不涉及界面重新设计。
...
- getScriptJson API扩展：完善现有获取剧本JSON数据的接口，支持生成浏览器可直接访问的.json链接
- 响应格式标准化
...
### JSON复制功能完善
- 修改getScriptJson云函数：完善现有云函数，支持生成浏览器可直接访问的.json链接
- script-detail.vue：优化复制JSON按钮的链接生成逻辑（按钮UI已存在）
- 完善剪贴板复制功能和浏览器链接生成
